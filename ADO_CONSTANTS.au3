...
#WARRNING: $ADO_ERR_RECORDSETEMPTY And $ADO_ERR_NOCURRENTRECORD must be at the end of @ERROR lists
#WARRNING: This is because @error should be managed like in the following example concluded between #cs #ce
#cs proper management way of @error returned by ADO.au3 Functions
	Local $vResult = _SQL_Execute(.....)
	If @error >= $ADO_ERR_RECORDSETEMPTY Then
		; not an error just empty results - just manage this here
	ElseIf @error Then
		; show error message to End User
	Else
		; process the acquired data
	EndIf
#ce proper management way of @error returned by ADO.au3 Functions

Global Enum _
		$ADO_EXT_DEFAULT, _ ;				default Extended Value
		$ADO_EXT_PARAM1, _ ;				Error Occurs in Parameter #1
		$ADO_EXT_PARAM2, _ ;				Error Occurs in Parameter #2
		$ADO_EXT_PARAM3, _ ;				Error Occurs in Parameter #3
		$ADO_EXT_PARAM4, _ ;				Error Occurs in Parameter #4
		$ADO_EXT_PARAM5, _ ;				Error Occurs in Parameter #5
		$ADO_EXT_PARAM6, _ ;				Error Occurs in Parameter #6
...
; RecordOpenOptionsEnum - default is $ADO_adOpenRecordUnspecified
; https://msdn.microsoft.com/en-us/library/ms676803(v=vs.85).aspx
...
; SaveOptionsEnum
; https://msdn.microsoft.com/en-us/library/office/jj249053.aspx
Global Const $ADO_adSaveCreateNotExist = 1
Global Const $ADO_adSaveCreateOverWrite = 2

; SchemaEnum
; https://msdn.microsoft.com/pl-pl/library/ms675274(v=vs.85).aspx
